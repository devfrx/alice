        ...
        <div v-else class="search-panel__results">
          ...
          >
            ...
            <Transition name="scrape-expand">
              <pre
                v-if="expandedMap[i] && scrapedTexts[i]"
                class="search-panel__scraped"
              >{{ scrapedTexts[i] }}</pre>
            </Transition>
          </article>
        </div>
...
.search-panel__title { font-weight: 600; font-size: 14px; }
.search-panel__badge {
...
.search-panel__close:hover { background: rgba(255,255,255,0.08); }
.search-panel__input-wrap { padding: 10px 14px 6px; }
.search-panel__input {
...
.search-panel__body { flex: 1; overflow-y: auto; padding: 8px 14px; }
.search-panel__status {
  text-align: center; padding: 32px 0;
  color: var(--text-secondary, #aaa); display: flex;
  align-items: center; justify-content: center; gap: 8px;
}
.search-panel__status--error { color: var(--accent, #e94560); }
...
.search-panel__card:hover { border-color: var(--accent, #e94560); }
.search-panel__card-title {
  font-weight: 600; color: var(--accent, #e94560);
  cursor: pointer; text-decoration: none; display: block;
  margin-bottom: 2px; line-height: 1.3;
}
.search-panel__card-title:hover { text-decoration: underline; }
...
.search-panel__card-snippet {
  margin: 0; font-size: 12px; color: var(--text-secondary, #aaa);
  display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical;
  overflow: hidden;
}
.search-panel__card-actions { display: flex; gap: 6px; margin-top: 8px; }
.search-panel__btn {
  padding: 3px 8px; border-radius: 4px; border: none; font-size: 11px;
  background: var(--bg-tertiary, #0f3460); color: var(--text-primary, #eee);
  cursor: pointer; transition: opacity 0.15s;
}
.search-panel__btn:hover { opacity: 0.8; }
.search-panel__btn:disabled { opacity: 0.5; cursor: wait; }
.search-panel__btn--accent { background: var(--accent, #e94560); color: #fff; }
.search-panel__scrape-error {
  margin-top: 6px; font-size: 11px;
  color: var(--accent, #e94560);
}
.search-panel__scraped {
  margin: 8px 0 0; padding: 8px; border-radius: 4px;
  background: var(--bg-primary, #1a1a2e); font-size: 11px;
  max-height: 200px; overflow-y: auto; white-space: pre-wrap;
  word-break: break-word; color: var(--text-secondary, #aaa);
}
...
/* Scrape expand */
.scrape-expand-enter-active, .scrape-expand-leave-active {
  transition: max-height 0.2s ease, opacity 0.2s;
}
.scrape-expand-enter-from, .scrape-expand-leave-to {
