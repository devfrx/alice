...
3. **Tools & the turn executor** — plugins expose `ToolDefinition`s (JSON-serializable, fed to the LLM as function-calling tools). `core/tool_registry.py` aggregates them. A user message becomes a **turn**, executed through the strategy layer in `services/turn/`:
   - `create_turn_executor()` (`turn/factory.py`) picks the implementation.
   - `DirectTurnExecutor` — the default single-shot path (LLM ↔ tool loop). This is the production path; agent mode is **off by default**.
   - `AgentTurnExecutor` — selected only when `agent.enabled` is true and `AgentComponents` are present. Wraps the classifier → planner → critic loop in `services/agent/`.
   - The underlying tool-call loop lives in `api/routes/_tool_loop.py` (`run_tool_loop`).
