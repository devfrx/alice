...
const overallStatus = computed<ServiceStatus>(() => {
    ...
    if (counts.down > 0 && counts.up === 0 && counts.degraded === 0 && counts.starting === 0) return 'down'
...
<template>
    <header class="titlebar">
        <div class="titlebar__left">
            <button class="titlebar__menu-btn" type="button" aria-label="Apri sidebar"
                :aria-pressed="uiStore.sidebarOpen" title="Sidebar" @click="uiStore.toggleSidebar">
        ...
        <div class="titlebar__center">
            <div class="titlebar__service-menu">
                <button ref="serviceTriggerRef" class="titlebar__service-trigger" :class="statusClass(overallStatus)"
                    ...
                    <AppIcon name="chevron-down" :size="12" class="titlebar__service-chevron"
                ...
                <UiPopover :open="serviceAccordionOpen" :anchor-el="serviceTriggerRef" placement="bottom" align="center"
                    ...
                    <section id="titlebar-services-panel" class="titlebar__service-panel">
                        <header class="titlebar__service-panel-head">
                            ...
                            <button class="titlebar__refresh-btn" type="button" aria-label="Aggiorna servizi"
                                title="Aggiorna" :disabled="refreshingServices" @click.stop="refreshServiceStatus">
                                <AppIcon name="refresh-cw" :size="13" :class="{ 'is-spinning': refreshingServices }" />
                            </button>
                        </header>
                        ...
                        <div v-if="refreshingServices && serviceRows.length === 0" class="titlebar__service-empty">
                            <AliceSpinner size="sm" />
                            <span>Caricamento servizi...</span>
                        </div>

                        <ul v-else-if="serviceRows.length" class="titlebar__service-list">
                            <li v-for="service in serviceRows" :key="service.name" class="titlebar__service-row"
                                ...
                                <span class="titlebar__service-row-main">
                                    ...
                                    <span class="titlebar__service-row-meta">
                                        ...
                                        <span v-if="service.detail" class="titlebar__service-row-detail"
                        ...
                        <footer class="titlebar__model-footer">
                            ...
                            <span class="titlebar__model-name"
                                :class="{ 'titlebar__model-name--empty': !hasActiveModel }" :title="modelDisplayName">
                                {{ modelDisplayName }}
                            </span>
                        </footer>
                    </section>
        ...
        <div class="titlebar__right">
            <div class="titlebar__controls">
                <button class="titlebar__btn titlebar__btn--minimize" type="button" aria-label="Minimize"
                    @click="handleMinimize">
                    <AppIcon name="win-minimize" :size="10" />
                </button>
                <button class="titlebar__btn titlebar__btn--maximize" type="button"
                    :aria-label="isMaximized ? 'Restore' : 'Maximize'" @click="handleMaximize">
                    <AppIcon v-if="!isMaximized" name="win-maximize" :size="10" />
                    <AppIcon v-else name="win-restore" :size="10" />
                </button>
                <button class="titlebar__btn titlebar__btn--close" type="button" aria-label="Close"
                    @click="handleClose">
