...
2. **Plugin system (`core/plugin_base.py`, `core/plugin_manager.py`)** — every capability (web search, calendar, memory, email, MCP client, CAD/chart/3D generators, the agent loop, etc.) is a `BasePlugin` subclass under `backend/plugins/<name>/plugin.py`. Discovery is **static**: each plugin module assigns `PLUGIN_REGISTRY["name"] = MyPlugin` at import time (required for PyInstaller bundling — there is no dynamic filesystem scan in production). The manager resolves dependencies via topological sort, initializes in order, creates plugin-owned DB tables (`get_db_models`), then calls `on_app_startup`. Which plugins load is driven by `config/default.yaml` `plugins.enabled`, then overridden by persisted per-user toggles in the DB.

3. **Tools & the turn executor** — plugins expose `ToolDefinition`s (JSON-serializable, fed to the LLM as function-calling tools). `core/tool_registry.py` aggregates them. A user message becomes a **turn**, executed through the strategy layer in `services/turn/`. `create_turn_executor()` (`turn/factory.py`) picks one implementation from `ctx.config.agent`:
   - `DirectTurnExecutor` — the engine: streams the LLM and drives the tool-call loop (`api/routes/_tool_loop.py` `run_tool_loop`, which already handles step budget, per-iteration context compaction, tool dedup, confirmation). Used directly for the **lite** path (`agent.enabled=false`, voice mode) **and** as the **model-driven default**.
   - **Model-driven agentic chat is the default** (`agent.enabled=true`, `agent.structured_mode=false`): it *is* `DirectTurnExecutor` plus the `agent` plugin's meta-tools — `update_plan` (a mutable todo-list) and `spawn_subagent` (isolated-context delegation). The model drives its own plan/delegation inside the normal loop; there is no separate pre-planning pass.
   - `ReflectiveTurnExecutor` — wraps the model-driven path with a single, non-blocking final-answer self-check (reuses `CriticService`). Selected only when `agent.reflection.enabled` (off by default).
   - `AgentTurnExecutor` (`services/agent/`) — the **opt-in** legacy structured pipeline: classifier → planner → critic. Selected only when `agent.structured_mode=true`. Preserved for structured/traceable tasks; not on the default path.
   - Config lives under a single `agent.*` tree: `enabled`, `structured_mode`, `planning`, `delegation`, `reflection.{enabled,tool_turns_only}`, `subagent.*`, plus the structured-mode knobs (`classifier`/`planner`/`critic`/`persistence`, used only when `structured_mode`).
...
- State is **Pinia** stores (`renderer/src/stores/`: `chat`, `voice`, `settings`, `plugins`, `artifacts`, `memory`, `calendar`, `email`, `whiteboard`, `mcp`, `services`, `ui`). Backend comms are centralized in `renderer/src/services/api.ts` (REST) and `ws.ts` (WebSocket).
