...
<template>
  <div
    v-if="contextInfo || isCompressing"
    ref="ctxRef"
    class="ctx"
  ...
  >
    ...
    <template v-else-if="contextInfo">
      <svg
        class="ctx__ring"
        :class="{ 'ctx__ring--pulse': shouldPulse, 'ctx__ring--pulse-fast': pulseFast }"
        viewBox="0 0 20 20"
        fill="none"
        aria-hidden="true"
      >
      ...
      <span
        class="ctx__pct"
        :style="{ color: fillPercent >= 60 ? fillColor : 'var(--text-muted)' }"
      >
        {{ prefix }}{{ fillPercent }}%
      </span>
    ...
    <Teleport to="body">
      <div
        v-if="isHovered && contextInfo && !isCompressing"
        class="alice-ctx-tip"
