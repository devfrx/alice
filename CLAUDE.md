...
4. **Scope & permission modes** — every conversation runs under a **workspace scope** (`services/scope_service.py`, store `scope.ts`): filesystem/exec tools are confined to an explicit folder or an ephemeral sandbox (confinement, not denial). The scope folders also reach MCP servers via the **MCP roots capability** (`services/mcp_session.py` + plugin `mcp_client`: roots = the server's static launch dirs ∪ the global union of all conversation scopes, re-notified live on every `scope.updated` and replayed once at startup — the server boundary is process-global, NOT per-conversation). On top, a **four-tier permission mode** per conversation (`services/permission_mode_service.py`, `permission_mode_policy.py`, `permission_rules.py`; store `permissionMode.ts`): `strict` (prompt for everything), `auto_edits` (auto-approve safe in-scope writes), `plan` (read-only), `autopilot` (full autonomy with circuit-breaker guards). A confirmation's "remember" choice (`conversation`/`persistent`) is persisted by the engine as a `PermissionRule` (conversation-scoped or global) AFTER the batch checkpoint — never mid-UoW (SQLite single-writer). Since Fase 2 Mossa 1, **MCP tools are inside the capability model**: `services/mcp_tool_mapping.py` maps the server's MCP tool annotations onto the gate (`readOnlyHint=true` → `mcp_read`/safe/no-confirm; otherwise `mcp_write` with `destructiveHint` deciding medium vs dangerous, always confirmed; missing annotations or `trust_annotations: false` → conservative fallback `mcp_write`/dangerous/confirmed; plan tier denies and withholds `mcp_write`). The annotations **provenance** is preserved on `ToolDefinition.mcp` (`McpToolMeta` — informational, never read by the gate; can diverge from the gate classification in the `path_args` fail-closed branch) and surfaces in the UI: the confirmation dialog shows an MCP-origin badge and an explicit fallback warning, the MCP settings panel shows per-server `trust_annotations` and the derived per-tool level, and the `/api/mcp/*` routes are typed with closed vocabularies. Per-server `mcp.servers[].path_args` promotes declared path-carrying tools to real `fs_read`/`fs_write` + `path_args` — fail-closed if a declared arg is missing from the tool's `inputSchema` — so the per-conversation scope confinement applies by construction. The in-memory grant layer was removed (PermissionRules on DB are the only override); path validation is consolidated in `core/path_safety.py`. Config under `permissions.*`. Tool handlers must respect both gates.
