...
watch(
  ...
  async (visible) => {
    if (visible) {
      ...
      const btn = cardRef.value?.querySelector<HTMLElement>('.modal__btn--confirm, .modal__btn--ok')
      btn?.focus()
    } else {
      unlockBodyScroll()
    ...
    <Transition name="modal">
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="state.title || 'Dialogo'"
          :aria-describedby="isCustom ? undefined : 'modal-desc'"
