...
const CADGenerationPlaceholder = defineAsyncComponent(
  () => import('./CADGenerationPlaceholder.vue')
)
...
<template>
  <div
    ...
    role="article"
    :aria-label="`Messaggio ${message.role}`"
  >
    ...
    <div class="bubble" :class="bubbleClass">
      ...
      <div v-if="isContextSummary" class="context-summary">
        <button class="context-summary__toggle" @click="summaryCollapsed = !summaryCollapsed">
          ...
          <AppIcon
            ...
            :size="12"
            :stroke-width="1.5"
            class="context-summary__chevron"
            :class="{ 'context-summary__chevron--open': !summaryCollapsed }"
          />
        </button>
        <div
          class="context-summary__body"
          :class="{ 'context-summary__body--collapsed': summaryCollapsed }"
        >
          <!-- eslint-disable vue/no-v-html -- sanitized markdown render (markdown-it html:false) -->
          <div
            class="context-summary__content"
      ...
      <CADGenerationPlaceholder
        v-if="showInlineGenerationPlaceholder && cadGenerationInProgress"
        compact
      ...
      <CADViewer
        v-if="cadPayload"
        :model-url="cadPayload.export_url"
        :model-name="cadPayload.model_name"
      ...
      <div v-if="whiteboardPayload" class="whiteboard-card">
        <AppIcon
          name="whiteboard-card"
          :size="14"
          :stroke-width="1.5"
          class="whiteboard-card__icon"
        />
      ...
      <template v-if="isPlainToolResult">
        <button class="tool-result__toggle" @click="toolResultCollapsed = !toolResultCollapsed">
          ...
          <span v-if="toolResultCollapsed" class="tool-result__preview">{{
            toolResultPreview
          }}</span>
          <AppIcon
            name="chevron-down"
        ...
        <div
          ...
          :class="{ 'tool-result__body--collapsed': toolResultCollapsed }"
        >
          <div class="tool-result__inner">
            <!-- eslint-disable-next-line vue/no-v-html — content is sanitised by markdown-it -->
            <div class="bubble__content" @click="handleCodeBlockClick" v-html="htmlContent" />
          </div>
        </div>
      </template>
      ...
      <MessageVersionNav
        v-if="hasVersions && message.role === 'user' && message.version_group_id"
