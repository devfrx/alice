...
const visibleItems = computed(() => {
  return artifactsStore.items.filter((a) => {
    ...
    if (conversationFilter.value !== 'all' && a.conversation_id !== conversationFilter.value)
      return false
    return true
  })
})
...
const isFiltered = computed(
  () => kindFilter.value !== 'all' || pinnedOnly.value || conversationFilter.value !== 'all'
)
...
<template>
  <section class="board-view" aria-label="Bacheca artefatti">
    ...
    <div class="board-view__body">
      <div
        v-if="artifactsStore.loading && !artifactsStore.items.length"
        class="board-view__loading"
      >
