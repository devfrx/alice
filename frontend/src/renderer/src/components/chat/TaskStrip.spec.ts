...
// The tasks store imports `{ api }` from services/api; stub just getTasks so the
// store's ensureForConversation/fetch resolve without reaching a backend.
...
function createStripState(opts: {
  ...
  const api_ = scope.run(() => {
    const steps = computed(() =>
      conversationId.value ? tasks.tasksFor(conversationId.value) : [],
    )
    ...
    const allComplete = computed(
      () => total.value > 0 && completed.value === total.value,
    )
...
function seed(tasks: ReturnType<typeof useTasksStore>, cid: string, steps: TaskStep[]) {
...
describe('TaskStrip.vue source structure', () => {
  const src = readFileSync(
    fileURLToPath(new URL('./TaskStrip.vue', import.meta.url)),
    'utf8',
  )
