...
> **Esito (2026-07-11):** DONE. Commit `d2ebd59` (package `commands/` + install App.vue + call-site AppSidebar + 10 test). Spec review: conforme al 100% (5 comandi esatti, exposeToAgent mai settato, barrel esatto, useRouter rimosso da AppSidebar dopo grep, scope pulito). Quality review (top, completata in due riprese per limite di sessione): "Approved with nits" — seam fase-7 giudicata solida (metadata serializzabili, `any` confinato, default anti-escalation TESTATO); trovati: (1) il guard HMR `has('view.switch')` mascherava handler STANTII quando si edita core.ts (il singleton sopravvive, il guard salta la re-install); (2) drift error-path di onHome (prima navigava comunque su create fallita); (3) comandi store-backed non testati. Fix del CONTROLLER in `776364f`: install reso IDEMPOTENTE (unregister dei CORE_COMMAND_NAMES in testa, guard rimosso da App.vue), fallback `view.switch` nel catch di onHome, +3 test (conversation.open happy/failure con parità no-nav-su-load-fallito, idempotenza re-install), push uniformato a `{name:'assistant'}`, commenti su sidebar.toggle capability e query artifact.show. Gate: typecheck verde, 20 file/153 test verdi, 0 errori eslint. Nit registrati per fase 7: campo `description` machine-facing nel manifest; validazione runtime argsSchema OBBLIGATORIA al bridge (execute non valida).
...
> **Esito (2026-07-11):** DONE in due commit: `f15ef57` (frame `artifact.bulk_deleted` + emissioni + parsers senza export_url + 2 test registry + regen + store actions + dispatcher + live-update in TldrawCanvas con echo-guard JSON e flag canvasReady + AgentTier alias) e `bd58974` (fix review). Spec review: conforme (51 test contratti+registry rilanciati verdi, regen giudicata genuina). Quality review (top): "Needs fixes" — trovati: (1) IMPORTANT la live-update NON scattava nel percorso Horizon (fallback su artifactsApi raw che non popola la cache dello store); (2) IMPORTANT rischio data-loss: il debounce 1500ms di tldraw-app.tsx non cancellato all'unmount poteva ri-PATCHare lo snapshot pre-agente (revert silenzioso + guard avvelenato) e scrivere cross-board su switch rapido; (3) finestra di mount che perdeva un update; (4) delete di conversazione con soli pinned non emetteva nulla malgrado il detach server-side; (5) fetchedConversations non pulita sul wipe; (6) naming `WsArtifactsBulkDeleted` incoerente coi fratelli singolari. Fix in `bd58974`: fallback via `fetchContent` (orphan-state allargato ai network error, giudicato PIÙ sicuro del canvas bianco), clearTimeout nel cleanup dell'effect (chiude entrambi gli scenari; residuo = solo PATCH già in volo, inerente), recheck one-pass post-mount (convergenza verificata), emissione su `unpinned or detached` + test pinned-only (101 test backend), `.clear()` sul wipe, rename `WsArtifactBulkDeleted` + seconda regen. Re-review: ✅ Approved. check-contracts PASS post-commit; boot ok. Backlog: orphan dead-end pre-esistente di TldrawCanvas (isOrphaned non resettato al cambio board); copy "deleted" per errori di rete.
