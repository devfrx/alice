...
3. **Tools & the turn executor** — plugins expose `ToolDefinition`s (JSON-serializable, fed to the LLM as function-calling tools). `core/tool_registry.py` aggregates them: it is a facade over the `core/tools/` components (catalog / availability / offer-policy / execution / RAG); the run-time permission gate stays in `services/permission_service.py`. A user message becomes a **turn**, executed through `services/turn/`. There is **one execution path**:
