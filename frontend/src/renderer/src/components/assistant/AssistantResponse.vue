...
const showToolCalls = computed(() => !props.isStreaming && props.toolCalls.length > 0)
...
<template>
  ...
  >
    ...
    <div
      ...
      :class="{ 'thinking-section--active': isThinkingPhase }"
    >
      <button class="thinking-toggle" @click="thinkingExpanded = !thinkingExpanded">
        <span class="thinking-toggle__icon">✦</span>
        <span
          class="thinking-toggle__label"
          :class="{ 'thinking-toggle__label--shimmer': isThinkingPhase }"
          >Ragionamento</span
        >
        <span v-if="isThinkingPhase" class="thinking-toggle__indicator" />
        <span
          class="thinking-toggle__chevron"
          :class="{ 'thinking-toggle__chevron--open': thinkingExpanded }"
          >›</span
        >
      </button>
    ...
    <!-- Reasoning thread: live trace during streaming, collapsed summary after -->
...
.alice-voice--thinking .alice-voice__emanation-line {
  height: 32px;
  opacity: 0.4;
  background: linear-gradient(to bottom, transparent, var(--thinking));
