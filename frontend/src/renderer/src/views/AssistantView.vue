...
const ChartViewer = defineAsyncComponent(() => import('../components/chat/ChartViewer.vue'))
...
watch(
  ...
  (id) => {
    if (id)
      artifactsStore.ensureForConversation(id).catch(() => {
        /* best-effort */
      })
  },
  { immediate: true }
...
const activeChart = computed(
  (): ChartPayload | null => chartPayloads.value[chartActiveIndex.value] ?? null
)
...
const activeWhiteboard = computed(
  (): WhiteboardPayload | null => whiteboardPayloads.value[whiteboardActiveIndex.value] ?? null
)

/** Auto-open the side panel on whiteboard tab when a new board appears. */
watch(
  () => whiteboardPayloads.value.length,
  (newLen, oldLen) => {
    if (newLen > oldLen) {
      sidePanelOpen.value = true
      sidePanelTab.value = 'whiteboard'
...
const visualAudioLevel = computed(() =>
  previewOrbState.value === 'listening' ? Math.max(audioLevel.value, 0.58) : audioLevel.value
)
...
const showLastResponse = computed(() => orbState.value === 'idle' || orbState.value === 'speaking')
  ...
  >
    ...
    <div class="assistant-view__main">
      <div class="assistant-view__center">
        ...
        <div class="assistant-view__content">
          ...
          <Transition name="greeting-fade">
            <div
              v-if="
                isConversationEmpty && !streamContent && !thinkingContent && orbState === 'idle'
              "
              key="greeting"
              class="assistant-view__greeting"
            >
          ...
          <Transition name="transcript-fade">
            <AssistantTranscript
              v-if="transcript || voiceStore.isListening || voiceStore.isProcessing"
              :text="transcript"
      ...
      <Transition name="toggle-fade">
        <button
          v-if="hasCadPanel && (!sidePanelOpen || sidePanelTab !== '3d')"
          class="assistant-view__3d-toggle"
          title="Apri pannello 3D"
          @click="
            () => {
              sidePanelOpen = true
              sidePanelTab = '3d'
        ...
        >
          ...
          <span v-if="cadModels.length > 1" class="assistant-view__3d-badge">{{
            cadModels.length
          }}</span>
        </button>
      </Transition>
      ...
      <Transition name="toggle-fade">
        <button
          v-if="hasCharts && (!sidePanelOpen || sidePanelTab !== 'chart')"
          class="assistant-view__chart-toggle"
        ...
        >
          ...
          <span v-if="chartPayloads.length > 1" class="assistant-view__chart-badge">{{
            chartPayloads.length
          }}</span>
        </button>
      </Transition>
      ...
      <Transition name="toggle-fade">
        <button
          v-if="hasWhiteboards && (!sidePanelOpen || sidePanelTab !== 'whiteboard')"
          class="assistant-view__wb-toggle"
          title="Mostra lavagne"
          @click="
            () => {
              sidePanelOpen = true
              sidePanelTab = 'whiteboard'
            }
          "
        >
    ...
    <Transition name="side-panel-slide">
      <div
        v-if="sidePanelOpen && (hasCadPanel || hasCharts || hasWhiteboards)"
        class="assistant-view__side-panel"
        :style="{ width: `${sidePanelWidth}px` }"
      >
        <!-- Resize drag handle -->
        <div class="side-panel__resize-handle" @mousedown="onResizeStart">
          <div class="side-panel__resize-grip"><span /><span /><span /></div>
        </div>
        <!-- Tab switcher (when multiple content types exist) -->
        <div v-if="sidePanelTabOptions.length > 1" class="side-panel__tabs">
          <UiSegmented
            class="side-panel__seg"
            size="sm"
            :model-value="sidePanelTab"
            :options="sidePanelTabOptions"
            aria-label="Contenuto pannello"
        ...
        <ImmersiveCADCanvas
          v-if="hasCadModels && (sidePanelTab === '3d' || (!hasCharts && !hasWhiteboards))"
          :models="cadModels"
          :active-index="cadActiveIndex"
          @update:active-index="
            (i) => {
              cadActiveIndex = i
            }
        ...
        <div
          v-if="cadGenerationInProgress && sidePanelTab === '3d'"
          class="side-panel__cad-loading"
        >
        ...
        <div
          v-if="hasCharts && (sidePanelTab === 'chart' || (!hasCadModels && !hasWhiteboards))"
          class="side-panel__chart-container"
        >
          <!-- Close button (only when no tab bar is shown) -->
          <button
            v-if="[hasCadPanel, hasCharts, hasWhiteboards].filter(Boolean).length <= 1"
            class="side-panel__chart-close"
          ...
          <div v-if="chartPayloads.length > 1" class="side-panel__chart-nav">
            <button
              class="side-panel__chart-nav-btn"
              :disabled="chartActiveIndex <= 0"
              @click="chartActiveIndex = Math.max(0, chartActiveIndex - 1)"
            >
              <AppIcon name="chevron-left" :size="14" />
            </button>
            <span class="side-panel__chart-counter"
              >{{ chartActiveIndex + 1 }} / {{ chartPayloads.length }}</span
            >
            <button
              class="side-panel__chart-nav-btn"
        ...
        <div
          v-if="hasWhiteboards && (sidePanelTab === 'whiteboard' || (!hasCadModels && !hasCharts))"
          class="side-panel__wb-container"
        >
          <!-- Close button (only when sole content) -->
          <button
            v-if="[hasCadPanel, hasCharts, hasWhiteboards].filter(Boolean).length <= 1"
            class="side-panel__wb-close"
            aria-label="Chiudi pannello"
            @click="closeSidePanel"
          >
            <AppIcon name="x" :size="14" />
          ...
          <div v-if="whiteboardPayloads.length > 1" class="side-panel__wb-nav">
            <button
              class="side-panel__chart-nav-btn"
              :disabled="whiteboardActiveIndex <= 0"
              @click="whiteboardActiveIndex = Math.max(0, whiteboardActiveIndex - 1)"
            >
              <AppIcon name="chevron-left" :size="14" />
            </button>
            <span class="side-panel__chart-counter"
              >{{ whiteboardActiveIndex + 1 }} / {{ whiteboardPayloads.length }}</span
            >
            <button
              class="side-panel__chart-nav-btn"
              :disabled="whiteboardActiveIndex >= whiteboardPayloads.length - 1"
              @click="
                whiteboardActiveIndex = Math.min(
                  whiteboardPayloads.length - 1,
                  whiteboardActiveIndex + 1
                )
              "
            >
              <AppIcon name="chevron-right" :size="14" />
